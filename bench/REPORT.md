...
## 8. ทดสอบ "ความลับ Divergence" จากเอกสาร SRISIAM Waves (เพิ่ม 2026-06-13)

สมมติฐานหน้า 13 ของเอกสาร: Div ยิ่งแรง + retrace ยิ่งตื้น → M3 ยิ่งไกล (4 กรณี)
วัดจากโครงสร้างจริง 801 ชุด (BTC/ETH, 4h+1h, 2020–2025) — `bench/divmatrix.js`:

| กลุ่ม | n | median M3 | เป้าตำรา |
|---|---|---|---|
| C1: Div≥161.8 + ret<38.2 (ตำราว่าดีสุด) | 48 | **114%** | 161.8–200% |
| C2: Div≥161.8 + ret 38–50 | 32 | 125% | 138.2–161.8% |
| C3: Div 138–162 + ret<38.2 | 33 | 126% | 100–138.2% |
| C4: Div 138–162 + ret 38–50 (ตำราว่าอ่อนสุด) | 31 | **128%** | 100% |

**ผล: ลำดับกลับด้านกับตำราเป๊ะ** · correlation divStrength↔M3 = -0.06, retraceShallow↔M3 = -0.18
(วัดแบบตัด bias เชิงกลไกแล้วก็ยังไม่เรียงตามตำรา) → matrix นี้ใช้ปรับ TP ไม่ได้
สิ่งเดียวที่ data สนับสนุน: "100 of A" เป็นเป้าทั่วไปที่สมเหตุสมผล (median ทะลุ M1 ~81–109% ทุกกลุ่ม)

## 9. Audit หน้า Shock Retest Scanner (เคลม "out-of-sample 2020–2025 +0.29R/ไม้")

Port logic ของ `shockretest.html` เป๊ะ (context กลับเทรนด์ + shock ≥1×ATR + entry 50%A +
structure stop + partial ครึ่งที่ TP1→BE + กรองฝั่งแข็งสวิงใหญ่ 1D) แล้วรันด้วย execution
จริง (SL-first, fee 0.1%, ถือทีละ setup) — `bench/shockretest-audit.js`:

| การทดสอบ | ผล |
|---|---|
| 2020–2025 (ช่วงที่เคลม +0.29R) | **+0.03R/ไม้** (n=639) — บวกแต่ต่ำกว่าเคลม ~10 เท่า |
| รายปี | กำไรกระจุกปี 2023 (+37.6R) · ปี 2020 = -26.1R |
| **2026 YTD (holdout จริง)** | **+0.21R/ไม้** (n=52, +11.0R) ← จุดที่น่าสนใจสุด |
| Random MC 1,000 รอบ | percentile 81 → ❌ ไม่ผ่าน p95 |
| Sensitivity | **เปราะ**: k1h 3→5 พลิก -29R → +44R · ไม่กรอง daily side → -11R |

อ่านผล: ระบบนี้**ดีขึ้นจาก KLAUD เดิมจริง** (structure stop + partial + daily filter ช่วยจริง —
ถอด filter แล้วติดลบทันที) และ 2026 YTD เป็นบวก แต่ (1) ตัวเลข +0.29R ไม่ผ่านการ replicate
ภายใต้ execution อนุรักษ์นิยม — ให้เทียบ 4 จุดใน COMPARE-SPEC กับ backtest ต้นทาง
(2) ความไวต่อ k1h ระดับพลิกเครื่องหมาย = ธงเตือน overfit (3) ยังแยกจากโชคไม่ได้ที่ p81
→ ข้อแนะนำ: เทรดได้เฉพาะ size เล็กแบบ forward test + log ทุกไม้ ยังไม่ใช่ระบบหลัก
